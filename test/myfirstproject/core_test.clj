...
(deftest db-test
  ...
  (is (not= () (db/q (name :persontest) "test3" "M" (c/to-sql-date "24-08-1994") "test_address" "addresss22")))
  (is (not= 0 (db/change :persontest {:name "Pavel"} ["name = ?" "test3"])))
  (is (not= () (db/q (name :persontest) "Pavel" "M" (c/to-sql-date "24-08-1994") "test_address" "addresss22")))
